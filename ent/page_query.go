...
func (pq *PageQuery) QueryBook() *BookQuery {
...
func (pq *PageQuery) QueryKeywords() *KeywordQuery {
	query := (&KeywordClient{config: pq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (pq *PageQuery) IDs(ctx context.Context) (ids []int, err error) {
...
func (pq *PageQuery) Clone() *PageQuery {
	...
	return &PageQuery{
		...
		withBook:     pq.withBook.Clone(),
		withKeywords: pq.withKeywords.Clone(),
...
func (pq *PageQuery) WithBook(opts ...func(*BookQuery)) *PageQuery {
	query := (&BookClient{config: pq.config}).Query()
...
func (pq *PageQuery) WithKeywords(opts ...func(*KeywordQuery)) *PageQuery {
	query := (&KeywordClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	pq.withKeywords = query
	return pq
}
...
func (pq *PageQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Page, error) {
	...
	if query := pq.withBook; query != nil {
		if err := pq.loadBook(ctx, query, nodes, nil,
			func(n *Page, e *Book) { n.Edges.Book = e }); err != nil {
			return nil, err
		}
	}
	if query := pq.withKeywords; query != nil {
		if err := pq.loadKeywords(ctx, query, nodes,
			func(n *Page) { n.Edges.Keywords = []*Keyword{} },
			func(n *Page, e *Keyword) { n.Edges.Keywords = append(n.Edges.Keywords, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *PageQuery) loadBook(ctx context.Context, query *BookQuery, nodes []*Page, init func(*Page), assign func(*Page, *Book)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Page)
	for i := range nodes {
		if nodes[i].book_pages == nil {
			continue
		}
		fk := *nodes[i].book_pages
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "book_pages" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (pq *PageQuery) loadKeywords(ctx context.Context, query *KeywordQuery, nodes []*Page, init func(*Page), assign func(*Page, *Keyword)) error {
	...
	query.Where(predicate.Keyword(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(page.KeywordsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.page_keywords
		if fk == nil {
			return fmt.Errorf(`foreign-key "page_keywords" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "page_keywords" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
